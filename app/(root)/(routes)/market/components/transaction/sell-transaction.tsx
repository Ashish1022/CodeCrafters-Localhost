...
export function SellTransaction({ company }: TransactionProps) {
  ...
  const foundStock = portfolioStocks.filter((item) => item.symbol.includes("TITAN"))
  const foundStockPrice = foundStock.length === 0 ? 0 : foundStock[0].shares * 3456.8
  ...
  const formSchema = z.object({
    prompt: z.coerce.number().int()
      .positive()
      .gt(0, {
        message: "Must be greater than Rs. 0.00"
      })
      .lte(price, {
      ...
      <SheetContent >
        ...
        <div className="grid gap-6 py-4">
          ...
          <div className="grid items-center grid-cols-4 gap-4">
            ...
            <Label className="col-span-3 pl-4" >
              {company.yahooStockV2Summary.price.longName}
            </Label>
          </div>
